	...
	<view class="content">
		<block v-if="brandInfo">
			<view class="headerBox" :style="'background-color:'+brandInfo.mini_color">
				...
				<view class="text" :style="'color:'+brandInfo.mini_color" v-if="brandInfo.mini_intro">
					<text></text><span>{{brandInfo.mini_intro}}</span>
				</view>
			</view>
			<view class="infoBox">
				<view class="title">开通&定制专属小程序</view>
				<view class="text">请联系您的专属客服</view>
				<view v-if="brandInfo.mini_phone" class="number" @click="getPhoneTap(brandInfo.mini_phone)"
					:style="'color:'+brandInfo.mini_color+';border-color:'+brandInfo.mini_color">
					{{brandInfo.mini_phone}}
				</view>
			</view>
			<view class="qrcode">
				<view v-if="brandInfo.mini_wx_img" class="label" style="margin-right: 50rpx;"
					:style="'background-color:'+brandInfo.mini_color+'10;border-color:'+brandInfo.mini_color+'60;'">
					<image @click="previewImage(0)" lazy-load :src="qrcodeList[0]" />
					<view class="text">{{brandInfo.mini_wx_title}}</view>
				</view>
				<view v-if="brandInfo.mini_kf_img" class="label"
					:style="'background-color:'+brandInfo.mini_color+'10;border-color:'+brandInfo.mini_color+'60;'">
					<image @click="previewImage(1)" lazy-load :src="qrcodeList[1]" />
					<view class="text">{{brandInfo.mini_kf_title}}</view>
				</view>
			</view>
			<view class="button_box" :style="'background-color:'+brandInfo.mini_color" @click="getMiniTap()">免费定制小程序
			</view>
		</block>
...
<script>
	export default {
		...
		onShareTimeline() {
			...
			return {
				title: `${self.brandInfo.mini_title}-${self.brandInfo.mini_intro}`,
				imageUrl: self.brandInfo.mini_logo
			}
		},
		methods: {
			getPhoneTap(phone) {
